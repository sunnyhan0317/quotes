...
router.get('/mood', async (req, res) => {
  try {
    ...
    const diaries = await MoodDiary.find(query).sort({ date: -1 }).limit(60);
    res.json(diaries);
  } catch (err) { res.status(500).json({ message: '伺服器錯誤' }); }
});

// 取得單一日期日記
router.get('/mood/:date', async (req, res) => {
  try {
...
router.post('/mood', async (req, res) => {
  try {
    ...
    const diary = await MoodDiary.findOneAndUpdate(
      ...
      { mood, moodScore, content, tags: tags || [], weather, updatedAt: Date.now() },
  ...
  try {
    ...
    for (const l of letters) {
      if (!l.opened && new Date(l.openDate) <= now) {
        l.opened = true; l.openedAt = now; await l.save();
      }
    }
    res.json(letters);
  } catch (err) { res.status(500).json({ message: '伺服器錯誤' }); }
...
router.post('/future', async (req, res) => {
  try {
    ...
    if (!title || !content || !openDate) return res.status(400).json({ message: '標題、內容和開封日期不能為空' });
    if (new Date(openDate) <= new Date()) return res.status(400).json({ message: '開封日期必須是未來的日期' });
